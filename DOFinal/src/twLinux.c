...
twSocket * twSocket_Create(const char * host, int16_t port, uint32_t options) {
   ...
   if ((rval = getaddrinfo(host, portStr, &hints, &p)) != 0) {
    ...
    for(; p != NULL; p = p->ai_next) {
        if ((res->sock = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
            continue;
        } else {
			foundServer = 1;
			///// res->addr = p;
...
int twSocket_Reconnect(twSocket * s) {
	if (!s) return -1;
	twSocket_Close(s);
	if ((s->sock = socket(s->addr.ai_family, s->addr.ai_socktype, s->addr.ai_protocol)) == -1) {
		return -1;
	}
        ...
        if(*p == '/' || *p == '\\') {
            *p = '\0';
            if (!twDirectory_FileExists(opath)) {
				if (!mkdir(opath, S_IRWXU | S_IRWXG | S_IRWXO)) if (errno != ENOENT) return errno;
			}
            *p = '/';
        }
	}
	/* Now we can add the desired directory */
	if (!mkdir(name, S_IRWXU | S_IRWXG | S_IRWXO)) return (errno != ENOENT) ? errno : 0;
	return 0;
}

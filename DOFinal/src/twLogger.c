...
char * CodeToString(enum msgCodeEnum m) {
	switch(m) {
	case	TWX_GET:
		return "GET";
	case	TWX_PUT:
...
char * EntityToString(enum entityTypeEnum m) {
	switch(m) {
	case	TW_THING:
		return "THING";
	case	TW_RESOURCE:
...
char * CharacteristicToString(enum characteristicEnum m) {
	switch(m) {
	case	TW_PROPERTIES:
		return "PROPERTIES";
	case	TW_SERVICES:
...
void logAuthBody( twAuthBody * b, char * buf, int32_t maxLength) {
	if (b && maxLength > 0) snprintf(buf, maxLength - 1, "Claim Count: 1\nName: %s\nValue: %s\n", b->name, b->value);
}
...
void logInfoTable(twInfoTable * it, char * buf, int32_t maxLength) {	
	if (it && it->ds && it->ds ->entries && maxLength > 0) {
		ListEntry * le = NULL;
...
void logRequestBody( twRequestBody * b, char * buf, int32_t maxLength) {
	if (b && maxLength > 0) {
		snprintf(buf, maxLength - 1, "EntityType: %s\nEntityName: %s\nCharacteristicType: %s\nCharateristicName: %s%s", 
			EntityToString(b->entityType),b->entityName,CharacteristicToString(b->characteristicType),b->characteristicName,  b->numHeaders ? "Headers:\n":"\n");
		maxLength = maxLength - strlen(buf);
		buf = buf + strlen(buf);
...
void twLogMessage (void * msg, char * preamble) {
	...
	bytesUsed = 6;
	sprintf(buf, "%s\nMessage Details:\nVersion: %d\nMethod/Code: 0x%x (%s)\nRequestID: %d\nEndpointID:%d\nSessionID: %d\nMultipart: %d\n",
		preamble ? preamble : "", m->version,m->code, CodeToString(m->code),m->requestId,m->endpointId,m->sessionId,m->multipartMarker);
...
void twLogHexString(const char * msg, char * preamble, int32_t length) {
	...
	if (tmp) {
		...
		twLog(TW_TRACE, "%s%s", preamble, tmp);
		TW_FREE(tmp);
	} else twLog(TW_ERROR, "twLogHexString: Error allocating storage");
}

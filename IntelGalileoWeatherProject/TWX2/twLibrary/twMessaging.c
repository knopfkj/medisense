...
char handleMessage(twMessage * msg) {
	...
	if (msg->multipartMarker) {
		if (msg->body) {
			...
			if (mp->chunkCount * mp->chunkSize > MAX_MESSAGE_SIZE) {
				TW_LOG(TW_WARN, "Multipart message would be too large. Discarding. chunkCount: %d, chunkSize: %d, max: %d", 
                        mp->chunkCount, mp->chunkSize, MAX_MESSAGE_SIZE);
	...
	*/
	if (!deleteOnReturn) twMessage_Delete(msg);
...
void twMessageHandler_msgHandlerTask(DATETIME now, void * params) {
	...
	if (incomingMsgList && msgHandlerSingleton) {
		...
		if (entry && entry->value) {
			...
			} else TW_LOG(TW_ERROR,"msgHandlerThread: NULL msg pointer found");
		}
		/* Remove this message from the list.  This also frees the message if needed. */
		twList_Remove(incomingMsgList, entry, deleteMsg);
...
int msgHandlerOnBinaryMessage (struct twWs * ws, const char *at, size_t length) {
	...
	s = twStream_CreateFromCharArrayZeroCopy(at, length);
	msg = twMessage_CreateFromStream(s);
	twStream_Delete(s);
	/* Create the message from the incoming data */
	if (!msg) {
		/* Message not created successfully */
		TW_LOG(TW_ERROR, "msgHandlerOnBinaryMessage: Error creating message from stream");
		return 1;
	}
	TW_LOG(TW_TRACE,"msgHandlerOnBinaryMessage: Received Binary Message ID: %d", msg->requestId);
	if (!incomingMsgList) {
...
twMessageHandler * twMessageHandler_Instance(twWs * ws) {
	...
	tmp->responseCallbackList = twList_Create(0);
	...
	/******************
	tmp->multipartMessageList = twList_Create(twMessage_Delete);
	if (!tmp->multipartMessageList) {
		TW_LOG(TW_ERROR, "twMessageHandler_Instance: Error creating multipart message list");
		twMessageHandler_Delete(tmp);
		return 0;
	}
	*********************/
